...
class Courses with ChangeNotifier {
  ...
  Future<void> buyCourse(String productId, courseId) async {
    try {
      ...
      bool isBought = customerInfo.allPurchasedProductIdentifiers
          .where((element) => element == productId)
          .isNotEmpty;

      if (isBought) {
        var customerId = customerInfo.originalAppUserId.toString();

        try {
          final url = Uri.parse(
              '$BASE_URL/api/payment_success_in_app_purchase/$authToken/$courseId/academybycreativeitem');
          ...
          if (responseData['message'] != 'success') {
            ...
            print(responseData);
            _courseDetailsitems
                .firstWhere((item) => item.courseId == courseId)
                .isPurchased = true;
      ...
      print(
          'An error occurred while purchasing the course: $error'); // Consider using a logger
    }
  }
